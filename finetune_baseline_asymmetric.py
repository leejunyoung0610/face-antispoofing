...
def evaluate_per_class(model: nn.Module, loader: DataLoader, device: torch.device):
    ...
    with torch.inference_mode():
        for batch in loader:
            x = batch["frame"].to(device)  # baseline uses normalized frame
    ...
    overall = (
        (correct_live + correct_spoof) / (total_live + total_spoof)
        if (total_live + total_spoof)
        else 0.0
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Fine-tune BaselineModel with asymmetric loss on Replay-Attack."
    )
    parser.add_argument(
        "--data_root",
    ...
    parser.add_argument("--fn_weight", type=float, default=3.0)
    parser.add_argument("--fp_weight", type=float, default=1.0)
    parser.add_argument(
        "--save_path",
        default="checkpoints/cross/baseline_asymmetric.pth",
    )
    ...
    print("Loading base weights: checkpoints/best_model.pth")
